    ...
    while bot_at_home < client.bots:
        ...
        if num != 0:
            ...
            if v == client.home:
                ...
                havebots[v] = num + havebots.get(v, 0)
                print(havebots)
            if bot_at_home + sum(havebots.values()) >= client.bots:
                if sum(havebots.values()) == 0:
                   break

                havebots[client.home] = bot_at_home
                print("steiner begins")
                print(havebots)
                order, b = bfs(approximation.steiner_tree(client.G, list(havebots.keys())), client.home)
                while order:
                    if bot_at_home >= client.bots:
                        break
                    (frm, to) = order.pop()
                    another = client.remote(frm, to)
                    if to == client.home:
                        bot_at_home += another
                print("steiner stops")
        else:
...
if __name__ == '__main__':
    ...
    a,b = bfs(approximation.steiner_tree(g, [0,0,0,2,2,2,2,5]), 0)
    d = {4:5, 5:1,1:7}
    while a:
        print(a.pop())
    print(b)
